...
If you're on Windows you can use WSL2 to build MixUpOS. Please have a look at the [Windows guide](BuildInstructionsWindows.md)
for details.
...
Run the following command to build and run MixUpOS:

```console
bash Meta/mixupos.sh run
```

This will compile all of MixUpOS and install the built files into the `Build/<architecture>/Root` directory inside your Git
repository. It will also build a disk image and start MixUpOS using QEMU. The chosen architecture defaults to
...
At this point you should have a fully functioning VM for MixUpOS. The [advanced build instructions guide](AdvancedBuildInstructions.md)
