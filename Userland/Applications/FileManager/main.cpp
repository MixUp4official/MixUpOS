...
static void do_extract_archive(Vector<ByteString> const& selected_file_paths, GUI::Window* window);
...
template<size_t N>
static bool path_has_any_suffix(StringView path, Array<StringView, N> const& suffixes)
{
    for (auto const& suffix : suffixes) {
        if (path.ends_with(suffix, CaseSensitivity::CaseInsensitive))
            return true;
    }
    return false;
}
...
template<size_t N>
static Optional<StringView> matching_suffix_for_path(StringView path, Array<StringView, N> const& suffixes)
{
    for (auto const& suffix : suffixes) {
        if (path.ends_with(suffix, CaseSensitivity::CaseInsensitive))
            return suffix;
...
static constexpr Array s_zip_like_archive_suffixes = {
    ".zip"sv,
    ".apk"sv,
    ".ipa"sv,
    ".jar"sv,
...
static constexpr Array s_tar_like_archive_suffixes = {
    ".tar.gz"sv,
    ".tar.bz2"sv,
    ".tar.xz"sv,
    ".tgz"sv,
...
static constexpr Array s_extractable_archive_suffixes = {
    ".deb"sv,
    ".tar.gz"sv,
    ".tar.bz2"sv,
    ".tar.xz"sv,
...
static bool is_extractable_archive_path(StringView path)
{
    return path_has_any_suffix(path, s_extractable_archive_suffixes);
}

static ByteString extraction_directory_for_archive(StringView archive_file_path)
{
    auto archive_suffix = matching_suffix_for_path(archive_file_path, s_extractable_archive_suffixes);
    if (!archive_suffix.has_value() || archive_file_path.length() <= archive_suffix->length())
        return ByteString::formatted("{}_contents", archive_file_path);
    return ByteString::copy(archive_file_path.substring_view(0, archive_file_path.length() - archive_suffix->length()));
}

static ErrorOr<void> run_command(ByteString const& executable, Vector<ByteString> const& arguments)
{
    ...
    if (!child_pid) {
        ...
        arg_list.append(executable.characters());
        for (auto const& argument : arguments)
            arg_list.append(argument.characters());
        arg_list.append(nullptr);

        int rc = execvp(executable.characters(), const_cast<char* const*>(arg_list.data()));
        if (rc < 0) {
            perror("execvp");
            _exit(1);
        }
    ...
    if (!WIFEXITED(status) || WEXITSTATUS(status) != 0)
        return Error::from_string_literal("Child process exited unsuccessfully");

    return {};
}

static ErrorOr<void> write_archive_member_to_file(StringView destination_path, ReadonlyBytes data, mode_t mode = 0644)
{
    auto parent_directory = LexicalPath(destination_path).parent();
    if (!parent_directory.is_empty())
        TRY(Core::Directory::create(parent_directory, Core::Directory::CreateDirectories::Yes));

    auto file = TRY(Core::File::open(destination_path, Core::File::OpenMode::Write | Core::File::OpenMode::Truncate, mode));
    TRY(file->write_until_depleted(data));
    return {};
}

static ErrorOr<void> extract_deb_package(StringView archive_file_path, StringView output_directory_path)
{
    if (!FileSystem::exists(output_directory_path))
        TRY(Core::System::mkdir(output_directory_path, 0755));

    auto package_file = TRY(Core::File::open(archive_file_path, Core::File::OpenMode::Read));
    auto package_data = TRY(package_file->read_until_eof());
    auto package_bytes = package_data.bytes();

    constexpr StringView ar_magic = "!<arch>\n"sv;
    if (package_bytes.size() < ar_magic.length() || StringView { reinterpret_cast<char const*>(package_bytes.data()), ar_magic.length() } != ar_magic)
        return Error::from_string_literal("Not a valid Debian package");

    size_t offset = ar_magic.length();
    while (offset + 60 <= package_bytes.size()) {
        auto header = package_bytes.slice(offset, 60);
        offset += 60;

        auto member_name = StringView { reinterpret_cast<char const*>(header.data()), 16 }.trim_whitespace();
        if (member_name.ends_with("/"sv))
            member_name = member_name.substring_view(0, member_name.length() - 1);

        auto member_size = StringView { reinterpret_cast<char const*>(header.slice(48, 10).data()), 10 }.trim_whitespace().to_number<size_t>();
        if (!member_size.has_value())
            return Error::from_string_literal("Malformed Debian package member");
        if (offset + member_size.value() > package_bytes.size())
            return Error::from_string_literal("Debian package member exceeds file size");

        auto member_data = package_bytes.slice(offset, member_size.value());
        offset += member_size.value();
        if ((offset % 2) != 0)
            ++offset;

        if (member_name.is_empty() || member_name == "/"sv || member_name == "//"sv)
            continue;

        auto member_path = LexicalPath::join(output_directory_path, member_name).string();
        TRY(write_archive_member_to_file(member_path, member_data));

        if (member_name == "debian-binary"sv)
            continue;

        if (member_name.starts_with("control.tar."sv)) {
            auto control_directory = LexicalPath::join(output_directory_path, "control").string();
            TRY(extract_archive(member_path, control_directory));
            continue;
        }

        if (member_name.starts_with("data.tar."sv)) {
            TRY(extract_archive(member_path, output_directory_path));
            continue;
        }
    }
...
static ErrorOr<void> extract_archive(StringView archive_file_path, StringView output_directory_path)
{
    if (!FileSystem::exists(output_directory_path))
        TRY(Core::System::mkdir(output_directory_path, 0755));

    if (archive_file_path.ends_with(".deb"sv, CaseSensitivity::CaseInsensitive))
        return extract_deb_package(archive_file_path, output_directory_path);

    if (path_has_any_suffix(archive_file_path, s_zip_like_archive_suffixes)) {
        Vector<ByteString> arguments;
        arguments.append("-d");
        arguments.append(output_directory_path);
        arguments.append(archive_file_path);
        return run_command("/bin/unzip"sv, arguments);
    }

    if (path_has_any_suffix(archive_file_path, s_tar_like_archive_suffixes)) {
        Vector<ByteString> arguments;
        arguments.append("-x");

        if (archive_file_path.ends_with(".tar.gz"sv, CaseSensitivity::CaseInsensitive) || archive_file_path.ends_with(".tgz"sv, CaseSensitivity::CaseInsensitive))
            arguments.append("-z");
        else if (archive_file_path.ends_with(".tar.bz2"sv, CaseSensitivity::CaseInsensitive) || archive_file_path.ends_with(".tbz"sv, CaseSensitivity::CaseInsensitive) || archive_file_path.ends_with(".tbz2"sv, CaseSensitivity::CaseInsensitive))
            arguments.append("-j");
        else if (archive_file_path.ends_with(".tar.xz"sv, CaseSensitivity::CaseInsensitive) || archive_file_path.ends_with(".txz"sv, CaseSensitivity::CaseInsensitive))
            arguments.append("-J");
        ...
        arguments.append("-C");
        arguments.append(output_directory_path);
        return run_command("/bin/tar"sv, arguments);
    }

    return Error::from_string_literal("Unsupported archive format");
}
...
ErrorOr<int> serenity_main(Main::Arguments arguments)
{
    ...
    if (!initial_location.is_empty()) {
        ...
        if (!FileSystem::is_directory(initial_location)) {
            if (is_extractable_archive_path(initial_location)) {
                auto temp_directory = FileSystem::TempFile::create_temp_directory();
                if (temp_directory.is_error()) {
                    dbgln("Failed to create temporary directory during archive extraction: {}", temp_directory.error());
                ...
                auto result = extract_archive(initial_location, temp_directory_path);
                if (result.is_error()) {
...
void do_extract_archive(Vector<ByteString> const& selected_file_paths, GUI::Window* window)
{
    ByteString archive_file_path = selected_file_paths.first();
    ByteString output_directory_path = extraction_directory_for_archive(archive_file_path);

    if (auto result = extract_archive(archive_file_path, output_directory_path); result.is_error())
        GUI::MessageBox::show(window, ByteString::formatted("Could not extract archive:\n{}", result.error()), "Extract Archive Error"sv, GUI::MessageBox::Type::Error);
}
    ...
    auto extract_archive_action
        = GUI::Action::create(
            ...
            [&](GUI::Action const&) {
                ...
                do_extract_archive(paths, directory_view->window());
            },
            window);
    ...
    directory_view->on_context_menu_request = [&](GUI::ModelIndex const& index, GUI::ContextMenuEvent const& event) {
        if (index.is_valid()) {
            ...
            if (node.is_directory()) {
                ...
                if (is_extractable_archive_path(node.full_path())) {
                    file_context_menu->add_action(extract_archive_action);
                    file_context_menu->add_separator();
                }
