...
static Array const s_title_gradient {
    ColorStop { Color(246, 250, 253, 78), 0.0f },
    ColorStop { Color(255, 255, 255, 166), 0.18f },
    ColorStop { Color(210, 228, 240, 128), 0.36f },
    ColorStop { Color(255, 255, 255, 120), 0.52f },
    ColorStop { Color(190, 214, 230, 140), 0.72f },
    ColorStop { Color(233, 243, 250, 102), 1.0f }
};

static constexpr struct {
    Color base { 244, 247, 250 };
    Color border { 108, 129, 148, 216 };
} s_frame_colors;
...
void GlassWindowTheme::paint_normal_frame(Painter& painter, WindowState window_state, WindowMode window_mode, IntRect const& window_rect, StringView window_title, Bitmap const& icon, Palette const& palette, IntRect const& leftmost_button_rect, int menu_row_count, bool window_modified) const
{
    ...
    if (!clipped_title_rect.is_empty()) {
        ...
        painter.draw_text(clipped_title_rect.translated(1, 2), window_title, title_font, title_alignment, Color(73, 96, 125, 220), Gfx::TextElision::Right);
        // FIXME: The translated(0, 1) wouldn't be necessary if we could center text based on its baseline.
        painter.draw_text(clipped_title_rect.translated(0, 1), window_title, title_font, title_alignment, Color(255, 255, 255, 245), Gfx::TextElision::Right);
    }
