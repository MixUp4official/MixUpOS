...
static NonnullRefPtr<GUI::Action> make_spawn_action(GUI::Window& window, ByteString const& title, StringView icon_path, StringView executable, ReadonlySpan<StringView> arguments = {})
{
    auto icon = Gfx::Bitmap::load_from_file(icon_path).release_value_but_fixme_should_propagate_errors();
    Vector<ByteString> argument_copies;
    argument_copies.ensure_capacity(arguments.size());
    for (auto const& argument : arguments)
        argument_copies.append(argument);

    return GUI::Action::create(title, icon, [&, executable_copy = executable.to_byte_string(), argument_copies = move(argument_copies)](auto&) {
        Vector<StringView> argument_views;
        argument_views.ensure_capacity(argument_copies.size());
        for (auto const& argument : argument_copies)
            argument_views.append(argument);
        GUI::Process::spawn_or_show_error(&window, executable_copy, argument_views.span());
    });
}
