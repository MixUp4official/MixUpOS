...
ErrorOr<NonnullRefPtr<HackStudioWidget>> HackStudioWidget::create(ByteString path_to_project)
{
    ...
    widget->m_export_mixapp_action = TRY(widget->create_export_mixapp_action());
...
ErrorOr<NonnullRefPtr<GUI::Action>> HackStudioWidget::create_export_mixapp_action()
{
    return GUI::Action::create("Export as .&mixapp...", [this](auto&) {
        auto source_file = active_file();
        if (source_file.is_empty()) {
            GUI::MessageBox::show_error(window(), "Please save the file first.");
            return;
        }

        bool is_python_file = source_file.ends_with(".py"sv);
        bool is_html_file = source_file.ends_with(".html"sv) || source_file.ends_with(".htm"sv);
        bool is_zip_file = source_file.ends_with(".zip"sv);
        bool is_jar_file = source_file.ends_with(".jar"sv);
        if (!is_python_file && !is_html_file && !is_zip_file && !is_jar_file) {
            GUI::MessageBox::show_error(window(), "CodeIt currently supports .py, .html/.htm, .zip, and .jar files for .mixapp export.");
            return;
        }
        ...
        auto source_absolute_path = m_project->to_absolute_path(source_file);
        LexicalPath source_path(source_absolute_path);
        auto suggested_output = LexicalPath::join(source_path.dirname(), ByteString::formatted("{}.mixapp", source_path.title())).string();
        auto mixapp_path = GUI::FilePicker::get_save_filepath(window(), source_path.title(), "mixapp"sv, suggested_output);
        if (!mixapp_path.has_value())
            return;

        auto output_directory = LexicalPath(mixapp_path.value()).dirname();
        auto relative_source = LexicalPath::relative_path(source_absolute_path, output_directory);
        ByteString type = "PythonScript";
        ByteString executable = "/bin/python3";
        ByteString entry_point {};
        if (is_html_file) {
            type = "HtmlDocument";
            ...
            auto selection = GUI::MessageBox::show(window(),
                "ZIP package detected.\n\nYes = Python ZIP app\nNo = HTML web ZIP app",
                "ZIP App Type"sv,
                GUI::MessageBox::Type::Question,
                GUI::MessageBox::InputType::YesNoCancel);
            if (selection == GUI::MessageBox::ExecResult::Cancel)
                return;
            if (selection == GUI::MessageBox::ExecResult::Yes) {
                type = "PythonZipPackage";
        ...
        builder.appendff("Source={}\n", relative_source);
        if (!entry_point.is_empty())
            builder.appendff("EntryPoint={}\n", entry_point);
        if (type == "JavaJar"sv) {
            builder.append("Argument1=-jar\n"sv);
            builder.appendff("Argument2={}\n", relative_source);
        } else {
            builder.appendff("Argument1={}\n", relative_source);
        }
        builder.appendff("Description=Generated by CodeIt from {}\n", source_path.basename());

        auto output_file = Core::File::open(mixapp_path.value(), Core::File::OpenMode::Write | Core::File::OpenMode::Truncate);
        if (output_file.is_error()) {
            GUI::MessageBox::show_error(window(), ByteString::formatted("Failed to write {}:\n{}", mixapp_path.value(), output_file.error()));
            return;
        }
        if (auto result = output_file.value()->write_until_depleted(builder.string_view().bytes()); result.is_error()) {
            GUI::MessageBox::show_error(window(), ByteString::formatted("Failed to write {}:\n{}", mixapp_path.value(), result.error()));
            return;
        }

        GUI::MessageBox::show(window(), ByteString::formatted("Exported {}", mixapp_path.value()), "mixapp Exported"sv, GUI::MessageBox::Type::Information);
    });
}
...
ErrorOr<void> HackStudioWidget::create_file_menu(GUI::Window& window)
{
    ...
    file_menu->add_action(*m_export_mixapp_action);
    file_menu->add_separator();
